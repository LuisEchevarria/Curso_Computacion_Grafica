/*
	Práctica Adicional. SkyBox    | Echevarria Aguilar Luis Angel
	Fecha de entrega: 05/05/2026  | No. Cuenta: 320236235
*/
...
// Variables para cada pata (reemplazan a FLegs y RLegs)
float FRLeg = 0.0f; // Front Right Leg
...
typedef struct _frame {
	...
	// Definición de elementos para las patas, la cola y la inclinación del cuerpo
	float tail;
	float tailInc;
	float FRLeg;
	float FRLegInc;
...
void saveFrame(void)
{
	...
	// Guardar el movimiento de la cola, las patas y la inclinación del cuerpo
...
void resetElements(void)
{
	...
	// Resetear el movimiento de la cola, las patas y la inclinación del cuerpo
...
void interpolation(void)
{
	...
	// Calcular el movimiento incremental de la cola, las patas y la inclinación del cuerpo
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "SkyBox - Echevarria Aguilar Luis Angel", nullptr, nullptr);
	...
	for (int i = 0; i < MAX_FRAMES; i++)
	{
		...
		// Inicializar la cola, las patas y la inclinación del cuerpo en los keyframes
		KeyFrame[i].tail = 0;
...
{
	//Dog Controls
	// Controles nuevos para la cola y las patas
	if (keys[GLFW_KEY_6]) tail += 0.1f;
	if (keys[GLFW_KEY_7]) tail -= 0.1f;

	if (keys[GLFW_KEY_Z]) FRLeg += 0.1f; // Pata delantera derecha
	if (keys[GLFW_KEY_X]) FRLeg -= 0.1f;

	if (keys[GLFW_KEY_C]) FLLeg += 0.1f; // Pata delantera izquierda
	if (keys[GLFW_KEY_V]) FLLeg -= 0.1f;

	if (keys[GLFW_KEY_B]) BRLeg += 0.1f; // Pata trasera derecha
	if (keys[GLFW_KEY_N]) BRLeg -= 0.1f;

	if (keys[GLFW_KEY_M]) BLLeg += 0.1f; // Pata trasera izquierda
	if (keys[GLFW_KEY_COMMA]) BLLeg -= 0.1f;

	// Controles para el eje Y (Subir/Bajar)
	if (keys[GLFW_KEY_I]) 
	{
		dogPosY += 0.0001f;
	}
	...
	// Controles para inclinar el cuerpo (Sentarse)
	if (keys[GLFW_KEY_8])
	{
		bodyPitch += 0.1f; 
	}
...
void Animation() {
	...
	if (play)
	{
		...
		else
		{
			...
			// Dibujar la cola, las patas y la inclinación del cuerpo con sus respectivos incrementos
